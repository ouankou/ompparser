...
void OpenMPAtomicDefaultMemOrderClause::generateDOT(std::ofstream &dot_file,
                                                    int depth, int index,
  ...
  std::string clause_kind = "atomic_default_mem_order_" +
...
void OpenMPInReductionClause::generateDOT(std::ofstream &dot_file, int depth,
  ...
  std::string clause_kind =
      "in_reduction_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
...
void OpenMPDepobjUpdateClause::generateDOT(std::ofstream &dot_file, int depth,
                                           int index, std::string parent_node) {
  ...
  std::string clause_kind =
      "update_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
...
void OpenMPDependClause::generateDOT(std::ofstream &dot_file, int depth,
                                     int index, std::string parent_node) {
  ...
  std::string clause_kind =
      "depend_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
...
void OpenMPAffinityClause::generateDOT(std::ofstream &dot_file, int depth,
                                       int index, std::string parent_node) {
  ...
  std::string clause_kind =
      "affinity_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
...
void OpenMPToClause::generateDOT(std::ofstream &dot_file, int depth, int index,
                                 std::string parent_node) {
  ...
  std::string clause_kind =
      "to_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
  ...
  if (mapper_identifier != "") {
    ...
    dot_file << current_line.c_str();
    current_line = indent + "\t" + mapper_identifier_name + " [label = \"" +
...
void OpenMPFromClause::generateDOT(std::ofstream &dot_file, int depth,
                                   int index, std::string parent_node) {
  ...
  std::string clause_kind =
      "from_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
...
void OpenMPDefaultmapClause::generateDOT(std::ofstream &dot_file, int depth,
                                         int index, std::string parent_node) {
  ...
  std::string clause_kind =
      "defaultmap_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
...
void OpenMPDeviceClause::generateDOT(std::ofstream &dot_file, int depth,
                                     int index, std::string parent_node) {
  ...
  std::string clause_kind =
      "device_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
...
void OpenMPDeviceTypeClause::generateDOT(std::ofstream &dot_file, int depth,
                                         int index, std::string parent_node) {
  ...
  std::string clause_kind =
      "device_type_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
...
void OpenMPTaskReductionClause::generateDOT(std::ofstream &dot_file, int depth,
                                            int index,
                                            std::string parent_node) {
  ...
  std::string clause_kind =
      "task_reduction_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
...
void OpenMPMapClause::generateDOT(std::ofstream &dot_file, int depth, int index,
                                  std::string parent_node) {
  ...
  std::string clause_kind =
      "map_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
...
void OpenMPDirective::generateDOT() {
  ...
  std::string filename = "OpenMPIR_" +
                         directive_kind.substr(0, directive_kind.size() - 1) +
                         ".dot";
...
void OpenMPDirective::generateDOT(std::ofstream &dot_file, int depth, int index,
                                  std::string parent_node,
  ...
  std::string directive_id =
      parent_node + "_" + directive_kind + "_" + std::to_string(index);
  current_line = indent + parent_node + " -- " + directive_id + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
  dot_file << indent << directive_id.c_str()
           << " [label = \"" + directive_kind + "\"]\n";

  // output score
  if (trait_score != "") {
    current_line =
        indent + directive_id + " -- " + directive_id + "_score" + "\n";
    dot_file << current_line.c_str();
    current_line = indent + "\t" + directive_id + "_score" +
                   " [label = \"score\\n " + trait_score + "\"]\n";
    dot_file << current_line.c_str();
  };
...
void OpenMPClause::generateDOT(std::ofstream &dot_file, int depth, int index,
                               std::string parent_node) {
  ...
  clause_kind = parent_node + "_" + clause_kind + "_" + std::to_string(depth) +
                "_" + std::to_string(index);
  current_line = indent + parent_node + " -- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
  current_line =
      indent + clause_kind + " [label = \"" + clause_label + +"\"]\n";
  dot_file << current_line.c_str();

  if (this->getKind() == OMPC_default) {
    OpenMPDirective *variant_directive =
        ((OpenMPDefaultClause *)this)->getVariantDirective();
...
void OpenMPReductionClause::generateDOT(std::ofstream &dot_file, int depth,
                                        int index, std::string parent_node) {
  ...
  std::string clause_kind =
      "reduction_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
...
void OpenMPDefaultClause::generateDOT(std::ofstream &dot_file, int depth,
  ...
  clause_string = parent_node + "_" + clause_string + "_" +
                  std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + " -- " + clause_string + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
...
void OpenMPOrderClause::generateDOT(std::ofstream &dot_file, int depth,
                                    int index, std::string parent_node) {
...
void OpenMPLastprivateClause::generateDOT(std::ofstream &dot_file, int depth,
                                          int index, std::string parent_node) {
  ...
  std::string clause_kind =
      "lastprivate_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
...
void OpenMPLinearClause::generateDOT(std::ofstream &dot_file, int depth,
                                     int index, std::string parent_node) {
  ...
  std::string clause_kind =
      "linear_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
...
void OpenMPAlignedClause::generateDOT(std::ofstream &dot_file, int depth,
                                      int index, std::string parent_node) {
  ...
  std::string clause_kind =
      "aligned_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
  std::string parameter_string;
  ...
  if (alignment != "") {
    ...
    dot_file << current_line.c_str();
    current_line = indent + "\t" + alignment_name + " [label = \"" +
                   alignment_name + "\\n " + parameter_string + "\"]\n";
    dot_file << current_line.c_str();
  };
};

void OpenMPScheduleClause::generateDOT(std::ofstream &dot_file, int depth,
                                       int index, std::string parent_node) {
  ...
  std::string clause_kind =
      "schedule_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
...
void OpenMPDistScheduleClause::generateDOT(std::ofstream &dot_file, int depth,
                                           int index, std::string parent_node) {
  ...
  std::string clause_kind =
      "dist_schedule_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
  ...
  if (chunk_size != "") {
    ...
    dot_file << current_line.c_str();
    current_line = indent + "\t" + chunk_size_name + " [label = \"" +
                   chunk_size_name + "\\n " + parameter_string + "\"]\n";
    dot_file << current_line.c_str();
  };
};

void OpenMPIfClause::generateDOT(std::ofstream &dot_file, int depth, int index,
                                 std::string parent_node) {
  ...
  std::string clause_kind =
      "if_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
...
void OpenMPInitializerClause::generateDOT(std::ofstream &dot_file, int depth,
                                          int index, std::string parent_node) {
  ...
  std::string clause_kind =
      "initializer_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
...
void OpenMPAllocateClause::generateDOT(std::ofstream &dot_file, int depth,
                                       int index, std::string parent_node) {
  ...
  std::string clause_kind =
      "allocate_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
...
void OpenMPAllocatorClause::generateDOT(std::ofstream &dot_file, int depth,
                                        int index, std::string parent_node) {
  ...
  std::string clause_kind =
      "allocator_" + std::to_string(depth) + "_" + std::to_string(index);
  current_line = indent + parent_node + "-- " + clause_kind + "\n";
  dot_file << current_line.c_str();
  indent += "\t";
