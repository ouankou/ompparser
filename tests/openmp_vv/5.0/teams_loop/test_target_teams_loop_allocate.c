#pragma omp target teams loop map(to: a[0:1024], b[0:1024]) map(from: c[0:1024]) uses_allocators(omp_low_lat_mem_alloc) thread_limit(1) allocate(omp_low_lat_mem_alloc: local) firstprivate(local)
